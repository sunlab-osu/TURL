...
def process_single_entity_table(input_table, config):
    pgEnt,pgTitle,secTitle,caption,headers,core_entities,all_entities,entities,entity_cand = input_table
...
def process_single_entity_table_CER(input_table, config):
    ...
    return [np.array(input_tok),np.array(input_tok_type),np.array(input_tok_pos),len(input_tok), \
...
class pretrain_entity_table_collate_fn_CER(pretrain_entity_table_collate_fn):
    def __call__(self, raw_batch):
        batch_input_tok,batch_input_tok_type,batch_input_tok_pos,batch_input_tok_length, \
        ...
        for i, (tok_l, ent_l) in enumerate(zip(batch_input_tok_length, batch_input_ent_length)):
            ...
            tmp_selected_core = random.sample(tmp_cand_core,2 if self.is_train else 1)
        ...
        batch_candidate_entity_set = torch.LongTensor(batch_candidate_entity_set)
        return batch_input_tok_padded, batch_input_tok_type_padded, batch_input_tok_pos_padded, \
