...
def train(args, config, train_dataset, model, eval_dataset = None):
    ...
    train_dataloader = EntityTableLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, max_entity_candidate=args.max_entity_candidate, \
                                        mlm_probability=args.mlm_probability, ent_mlm_probability=args.ent_mlm_probability)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            input_tok, input_tok_type, input_tok_pos, input_tok_labels, input_tok_mask, \
                input_ent, input_ent_type, input_ent_pos, input_ent_labels, input_ent_mask, candidate_entity_set = batch
...
def evaluate(args, config, eval_dataset, model, prefix=""):
    ...
    eval_dataloader = TableLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, max_entity_candidate=args.max_entity_candidate)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            tok_eval_acc += tok_acc
            ent_eval_acc += ent_acc
        nb_eval_steps += 1
